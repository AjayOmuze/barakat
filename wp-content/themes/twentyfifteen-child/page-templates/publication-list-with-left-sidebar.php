<?php
/**
 * Template Name: Publication List With Left Sidebar
 */
get_header();
?>
...
<section>
  ...
  <div class="blog_sec">
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-lg-9 col-md-9 col-sm-9 page-detail">
          ...
          <?php $presses = get_posts(array('post_type' => 'publication', 'posts_per_page' => -1)); ?>

          <?php if (is_array($presses) && count($presses) > 0): ?>
            <div class="row">
              <?php $i = 1;
              foreach ($presses as $post): setup_postdata($post); ?>
                <div class="col-md-3 col-sm-3 col-xs-6  col-sm-4 press_pd">
                  <?php $feat_image = wp_get_attachment_url(get_post_thumbnail_id($post->ID)); ?>
    <?php if ($feat_image != ''): ?>
    ...
    <?php endif; ?>
                  <h5><a href="<?php the_field('pdf', get_the_ID()) ?>" target="_blank"> THE BARAKAT COLLECTION</a></h5>
                  <p><?php the_title() ?></p>
                  <div class="clearfix">&nbsp;</div>
                </div>
